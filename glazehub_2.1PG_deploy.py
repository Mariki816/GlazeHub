...
def resetPword():
    tempPassword = request.form.get("newPassword")
    print "this is tempPassword", tempPassword
    tempPassword2 = request.form.get("passwordCheck")

    if tempPassword == tempPassword2:
        tempUserEmail = request.form.get("userEmail").lower()
        user = modelPG.User.getUserByEmail(tempUserEmail)
        hash = pbkdf2_sha256.encrypt(tempPassword,
...
def emailCPSend(userViewID, recipeName, batchSize):
    ...
    msg['From'] = gmail_user
    msg['To'] = "info@clay-planet.com"
    msg['CC'] = "glazehub@gmail.com"
    ...
    FROM = gmail_user
    TO = ["info@clay-planet.com", "glazehub@gmail.com"]
